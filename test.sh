...
echo "" 
useradd vt_admin
echo -e 'Vts_admin@2023\nVts_admin@2023' | passwd vt_admin


echo ""
echo "================================================================================"
echo "----------------------------- CREATE USER centos --------------------------------"
echo "================================================================================"
echo "" 
useradd centos
echo -e 'Vts_centos@2023\nVts_centos@2023' | passwd centos
...
if grep -q '^%wheel\s\+ALL=(ALL)\s\+ALL' /etc/sudoers; then
  sed -i 's/^%wheel\s\+ALL=(ALL)\s\+ALL/#%wheel ALL=(ALL) ALL/' /etc/sudoers #neu no tim thay doan text: %wheel all=(all) all thì nó sẽ thay thế %wheel thành #%wheel
  echo "Configuration updated."
else
  echo "Configuration is already set."
fi
...
#add them code cho phep traceroute, tcptraceroute, iotop ma khong can passwd, va se chay duoi quyen root
echo " ALL ALL=(root) NOPASSWD: /usr/bin/tcptraceroute,/bin/traceroute,/usr/sbin/iotop" | tee /etc/sudoers


# Doi permitRootLogin Yes -> permitRootLogin No: Khong cho root login qua SSH
...
#add them code: cho phep cac user ssh
echo "AllowUsers centos vt_admin" | tee /etc/ssh/sshd_config
...
#kiem tra cau hinh quay log, de tranh log co kich thuoc lon, chiem dung dung luong
cat >>./check_logrotate_config_syslog<<EOF
/var/log/cron
/var/log/maillog
/var/log/messages
/var/log/secure
...
check12=`diff check_logrotate_config_syslog /etc/logrotate.d/syslog`
if [[ -z check12 ]]
    then
        rm -rf check_logrotate_config_syslog
        echo "Nothing to do !"
    else
       mv check_logrotate_config_syslog /etc/logrotate.d/syslog -f
...
if [[ -z $check1 ]]
 ...
 #dong bo date voi may chu ntp viettel
echo "*/5 * * * * /sbin/ntpdate -u 192.168.181.50" | tee /var/spool/cron/root
